...
public class ImageSubmissionServiceTests
{
    ...
    {
        _awsS3ClientMock.UploadPetImageAsync(Arg.Any<MemoryStream>(), CreateUserRequest.Image, Arg.Any<string>())
            .Returns(S3FailImageResponse);

        async Task Result() => await _sut.UploadPetImageAsync(User.Id, CreateUserRequest.Image);
